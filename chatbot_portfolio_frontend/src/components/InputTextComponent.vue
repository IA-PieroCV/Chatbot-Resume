<template>
  <form class="flex bg-white rounded-full justify-between items-center w-5/12" @submit.prevent="sendHumanMessage">
    <IconChat class="h-8 ml-3 mr-2 text-gray-300" />
    <input v-model="modelValue" type="text" class="w-full h-12" :placeholder="rq" />
    <button :disabled="!messageStore.enabled_chat" class="h-12 rounded-r-full pl-2 pr-5 border-l-2" @click="sendHumanMessage">
